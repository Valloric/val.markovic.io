            ...
            for resource in node.resources:
                ...
                if created == "git":
                    if commits:
                        ...
                        created = _DateTimeFromFileTimestamp(os.path.getctime(resource.path))
                    resource.meta.created = created
                if modified == "git":
                    if commits:
                        modified = parse(commits[0].strip())
                    else:
                        modified = _DateTimeFromFileTimestamp(os.path.getmtime(resource.path))
                    resource.meta.modified = modified


def _DateTimeFromFileTimestamp(timestamp):
